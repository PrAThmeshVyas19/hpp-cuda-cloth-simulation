...
void initialize(void)
{
    ...
    // Generate cloth grid
    vmath::vec4 vertices[POINTS_TOTAL];
    GLuint indices[(POINTS_X - 1) * (POINTS_Y - 1) * 6]; // 6 indices per quad
    ...
    for (int y = 0; y < POINTS_Y; y++)
    {
        ...
        {
            float fx = (float)x / (float)(POINTS_X - 1) - 0.5f;
            float fy = (float)y / (float)(POINTS_Y - 1) - 0.5f;
            vertices[y * POINTS_X + x] = vmath::vec4(fx * 2.0f, fy * 2.0f, 0.0f, 1.0f);
